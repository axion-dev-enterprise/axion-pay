...
function snippet(endpoint: Endpoint, language: "curl" | "node") {
  ...
  if (endpoint.path === "/v1/merchant/webhooks" && endpoint.method === "POST") return language === "curl"
    ? `curl -X POST ${url} \\\n  -H "Authorization: Bearer $AXION_API_KEY" \\\n  -H "Content-Type: application/json" \\\n  -d '{"url":"https://seusite.com/webhooks/axion-pay","events":["payment.succeeded","payment.failed","subscription.created","subscription.renewed","subscription.past_due","subscription.canceled"]}'`
    : `const response = await fetch("${url}", {\n  method: "POST",\n  headers: {\n    Authorization: \`Bearer \${process.env.AXION_API_KEY}\`,\n    "Content-Type": "application/json"\n  },\n  body: JSON.stringify({\n    url: "https://seusite.com/webhooks/axion-pay",\n    events: ["payment.succeeded", "subscription.created", "subscription.renewed", "subscription.past_due", "subscription.canceled"]\n  })\n});\nconst { webhook } = await response.json();\nconsole.log("Secret:", webhook.secret);`;
